...
\end{itemize}
Hence constraints (\ref{independence constraints}) can be viewed as constraints (\ref{matching constraints}) together with some extra constraints on $(H,\prec)$. Let $\mathcal{O}(H)$ denote the set of all complete subgraphs of $H$ induced on three vertices. Then $\sigma(D)$ can be viewed as a linear system defined on preference system $(H,\prec)$ by the following inequalities:
\begin{alignat}{4}
